            ...
            for (let policyDetails of policies) {
                ...
                if (policy.type == exports.DEFINITION_TYPE) {
                    ...
                    if (azDefinition.error && azDefinition.error.code == POLICY_DEFINITION_NOT_FOUND) {
                        utilities_1.printPartitionedText(`Policy definition with id ${policy.id} does not exist in azure. A new definition will be created.`);
                        newPolicy = true;
                    }
                    else {
                    ...
                    if (azAssignment.error && azAssignment.error.code == POLICY_ASSIGNMENT_NOT_FOUND) {
                        utilities_1.printPartitionedText(`Policy assignment with id ${policy.id} does not exist in azure. A new assignment will be created.`);
                        newPolicy = true;
                    }
                    else {
                ...
                if (newPolicy) {
                    ...
                    policyDetails.operation = exports.POLICY_OPERATION_CREATE;
                    ...
                    if (updateRequired) {
                        policyDetails.policy = appendPolicyMetadata(policy, githubHash);
                        policyDetails.operation = exports.POLICY_OPERATION_UPDATE;
                        policyRequests.push(policyDetails);
                    }
                }
            }
