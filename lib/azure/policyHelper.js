...
function getPolicyDefinition(definitionPath) {
    ...
    if (!definition.policyRule && fileHelper_1.doesFileExist(policyRulesPath)) {
        const policyRule = fileHelper_1.getFileJson(policyRulesPath);
        definition.policyRule = policyRule;
    }
    if (!definition.parameters && fileHelper_1.doesFileExist(policyParametersPath)) {
        const policyParameters = fileHelper_1.getFileJson(policyParametersPath);
