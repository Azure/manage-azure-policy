...
function createRoleRequests(roleRequests) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        utilities_1.prettyDebugLog(`wait for 60s`);
        httpClient_1.sleepFor(60);
        try {
            const azHttpClient = new azHttpClient_1.AzHttpClient();
            yield azHttpClient.initialize();
            ...
            responses.forEach((response, index) => {
                if (response.httpStatusCode == httpClient_1.StatusCodes.CREATED) {
                    ...
                    utilities_1.prettyLog(`Role assignment could not be created related to assignment id ${roleRequests[index].policyAssignmentId}. Error : ${response.content.error.message}`);
                }
                else {
