    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            for (let policyDetails of policies) {
                ...
                githubHash = hashUtils_1.getObjectHash(policy);
                newPolicy = false;
                updateRequired = false;
                if (policy.type == exports.DEFINITION_TYPE) {
                    azPolicy = yield azHttpClient.getPolicyDefinition(policy);
                }
                else {
                    azPolicy = yield azHttpClient.getPolicyAssignment(policy);
                }
                if (azPolicy.error) {
                    // There was some error while getting the policy. Check if policy does not exist and needs to be created.
                    if ((azPolicy.error.code == POLICY_DEFINITION_NOT_FOUND && policy.type == exports.DEFINITION_TYPE)
                        || (azPolicy.error.code == POLICY_ASSIGNMENT_NOT_FOUND && policy.type == exports.ASSIGNMENT_TYPE)) {
                        utilities_1.printPartitionedText(`Policy with id : ${policy.id}, path : ${policyDetails.path} does not exist in azure. A new policy will be created.`);
                        newPolicy = true;
                    }
                    else {
                        // TODO : DO we need to throw here?
                        utilities_1.printPartitionedText(`Failed to get policy with id ${policy.id}, path ${policyDetails.path}. Error : ${JSON.stringify(azPolicy.error)}`);
                        continue;
                    }
                }
                else {
                ...
                if (!newPolicy) {
                    if (azureMetadata.GitHubPolicy) {
                        ...
                        if (azureHash == githubHash) {
                            utilities_1.printPartitionedText(`Hash is same for policy id : ${policy.id}`);
                            continue;
                        }
                        else {
                ...
                if (updateRequired || newPolicy) {
                    ...
                    policyDetails.operation = newPolicy ? exports.POLICY_OPERATION_CREATE : exports.POLICY_OPERATION_UPDATE;
...
function appendPolicyMetadata(policy, hash) {
