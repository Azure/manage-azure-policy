...
function getAllPolicyRequests(paths) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const allPolicyDetails = getAllPolicyDetails(paths);
...
function getAllPolicyDetails(paths) {
    let policies = [];
    let policy;
    const jsonPaths = fileHelper_1.getAllJsonFilesPath(paths);
...
function getPolicyOperationType(gitPolicyDetails, azurePolicy, currentHash) {
    ...
    if (azureMetadata[POLICY_METADATA_GITHUB_KEY]) {
        ...
        if (azureHash == currentHash) {
            utilities_1.prettyDebugLog(`Hash is same for policy id : ${gitPolicyDetails.policy.id}`);
            return exports.POLICY_OPERATION_NONE;
        }
        else {
            utilities_1.prettyDebugLog(`Hash is not same for policy id : ${gitPolicyDetails.policy.id}`);
            return exports.POLICY_OPERATION_UPDATE;
        }
    }
    else {
        utilities_1.prettyDebugLog(`GitHub metaData is not present for policy id : ${gitPolicyDetails.policy.id}`);
        return exports.POLICY_OPERATION_UPDATE;
    }
}
