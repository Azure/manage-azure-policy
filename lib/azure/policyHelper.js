...
exports.createUpdatePolicies = exports.getAllPolicyRequests = exports.POLICY_RESULT_FAILED = exports.POLICY_OPERATION_NONE = exports.POLICY_OPERATION_UPDATE = exports.POLICY_OPERATION_CREATE = exports.ASSIGNMENT_TYPE = exports.DEFINITION_TYPE = void 0;
...
const POLICY_METADATA_GITHUB_FILED = "GitHubPolicy";
const POLICY_METADATA_HASH_FIELD = "policy_hash";
function getAllPolicyRequests(paths) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            let allPolicyDetails = getAllPolicyDetails(paths);
            for (let policyDetails of allPolicyDetails) {
                gitPolicy = policyDetails.policy;
                currentHash = hashUtils_1.getObjectHash(gitPolicy);
                azurePolicy = yield getAzurePolicy(gitPolicy);
                if (azurePolicy.error && azurePolicy.error.code != POLICY_DEFINITION_NOT_FOUND && azurePolicy.error.code != POLICY_ASSIGNMENT_NOT_FOUND) {
                    ...
                    utilities_1.printPartitionedText(`Failed to get policy with id ${gitPolicy.id}, path ${policyDetails.path}. Error : ${JSON.stringify(azurePolicy.error)}`);
                    continue;
                }
                eventType = getPolicyEventType(policyDetails, azurePolicy, currentHash);
                if (eventType == exports.POLICY_OPERATION_CREATE || eventType == exports.POLICY_OPERATION_UPDATE) {
        ...
        for (const policyRequest of policyRequests) {
            switch (policyRequest.type) {
                case exports.DEFINITION_TYPE:
                    policyResults.push(yield upsertPolicyDefinition(azHttpClient, policyRequest));
                    break;
                case exports.ASSIGNMENT_TYPE:
                    policyResults.push(yield upsertPolicyAssignment(azHttpClient, policyRequest));
                    break;
            }
        }
...
function getAllPolicyDetails(paths) {
    let policies = [];
    let jsonPaths = fileHelper_1.getAllJsonFilesPath(paths);
...
function getWorkflowMetadata(policyHash) {
    let metadata = {
        ...
        run_url: utilities_1.getWorkflowRunUrl()
    };
    return metadata;
}
function getPolicyRequest(policyReq, hash, eventType) {
...
function getPolicyEventType(gitPolicyDetails, azurePolicy, currentHash) {
    if (azurePolicy.error) {
        utilities_1.printPartitionedText(`Policy with id : ${gitPolicyDetails.policy.id}, path : ${gitPolicyDetails.path} does not exist in azure. A new policy will be created.`);
    ...
    if (azureMetadata[POLICY_METADATA_GITHUB_FILED]) {
        ...
        if (azureHash == currentHash) {
            utilities_1.printPartitionedText(`Hash is same for policy id : ${gitPolicyDetails.policy.id}`);
            return exports.POLICY_OPERATION_NONE;
        }
        else {
        ...
        utilities_1.printPartitionedText("Github metaData is not present. Will need to update");
        return exports.POLICY_OPERATION_UPDATE;
    }
}
